...
const OrderDetails = ({orderId}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const getOrders = await get("/Data/Orders.json");

        const p = getOrders.find((order) => order._id === orderId);
        console.log('p',p);
          ...
          <div className="flex gap-5 flex-col lg:flex-row ">
            {/* cart details */}
              ...
              <div className="lg:basis-3/4 lg:border-r pr-5 border-dotted border-black overflow-auto w-full">
                <div className="text-4xl font-semibold pb-4">Products</div>
                ...
                <div className="grid grid-cols-4  w-[50rem] lg:w-full overflow-auto">
                  <div className="col-span-3">
                    Total Quantity: {order.totalItem}
                  </div>
                  <div className="flex flex-col gap-2">
                    <span className="border-b border-dotted py-2">
                      Delivery Cost: 50
                    </span>
                    <span>Subtotal: {(order.totalAmount + 50).toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <>
                ...
                <div className="form-control relative my-6">
                  <input
                    autoComplete="off"
                    id="country"
                    value={order?.shippingDetails?.country}
                    readOnly
                    name="country"
                    type="text"
                    className="peer placeholder-transparent h-10 w-full   bg-transparent text-black focus:outline-none focus:borer-rose-600 border-b-black/50 border-b"
                ...
                <div className="flex gap-2 w-full">
                  ...
                  <div className="form-control w-full relative my-6">
                    <input
                      ...
                      name="postal"
                      type="name"
                      value={order?.shippingDetails?.postal}
                    readOnly
                      className="peer placeholder-transparent h-10 w-full  bg-transparent text-black focus:outline-none focus:borer-rose-600 border-b-black/50 border-b"
                      placeholder="postal"
                      required
